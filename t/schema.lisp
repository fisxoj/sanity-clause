...
(deftest test-load-schema
  (let* ((schema (sanity-clause.schema:load (asdf:system-relative-pathname :sanity-clause/test "t/fixtures/schema.sexp")))
         (raw-data '(:port 3 :name "something" :mode "on" :boolean "no"))
         (data (sanity-clause.serde:load schema raw-data)))
